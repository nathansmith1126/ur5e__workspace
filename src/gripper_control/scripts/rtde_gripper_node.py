#!/usr/bin/env python3
# rtde_gripper_node.py
#
# ROS params:
#   ~robot_ip   (str, required) e.g. "172.22.22.2"
#   ~topic      (str, optional) default: "/gripper_cmd"
#
# Commands (std_msgs/String, case-insensitive):
#   "activate"
#   "open"
#   "close"
#   "set_force:<0-100>"
#   "set_speed:<0-100>"
#   "move:<mm>"           # move to opening in millimeters
#   "stop"                # calls rtde_c.stopRobot()
...
class RobotiqGripper:
    ...
    def __init__(self, address):
        ...
        self.connect(hostname=address, port=63352)
    ...
    def activate(self, auto_calibrate: bool = True):
        ...
        if not self.is_active():
            ...
            print("Gripper is already activated")
    ...
    def move(self, position: int, speed=None, force=None) -> Tuple[bool, int]:
    ...
    def move_and_wait_for_pos(self, position: int, speed=None, force=None) -> Tuple[int, ObjectStatus]:  # noqa
    ...
    def open(self, speed=None, force=None) -> Tuple[int, ObjectStatus]:
        return self.move_and_wait_for_pos(self._min_position, speed, force)
    
    def close(self, speed=None, force=None) -> Tuple[int, ObjectStatus]:
        return self.move_and_wait_for_pos(self._max_position, speed, force)

class GripperNode:
    def __init__(self):
        self.robot_ip = rospy.get_param("~robot_ip", None)
        if not self.robot_ip:
            rospy.logfatal("~robot_ip param is required")
            raise SystemExit(1)
        topic = rospy.get_param("~topic", "/gripper_cmd")

        # Connect RTDE and init gripper
        self.gripper = RobotiqGripper(self.robot_ip)
        # Do not auto-activate here; let user send "activate" explicitly

        rospy.on_shutdown(self._on_shutdown)
        rospy.Subscriber(topic, String, self._cb, queue_size=10)
        rospy.loginfo("gripper_node ready. Subscribed to %s", topic)

    def _cb(self, msg: String):
        cmd = msg.data.strip().lower()
        try:
            if cmd == "activate":
                self.gripper.activate()  # returns to previous position
            ...
            elif cmd.startswith("set_force:"):
                val = int(cmd.split(":", 1)[1])
                self.gripper.set_force(val)
                rospy.loginfo("Force set to %d%%", val)
                
            elif cmd.startswith("set_speed:"):
                val = int(cmd.split(":", 1)[1])
                self.gripper.set_speed(val)
                rospy.loginfo("Speed set to %d%%", val)
                
            elif cmd.startswith("move:"):
                mm = float(cmd.split(":", 1)[1])
                self.gripper.move(mm)
                rospy.loginfo("Move to %.2f mm", mm)
                
            # elif cmd == "stop":
            #     self.rtde_c.stopRobot()
            #     rospy.loginfo("RTDE program stopped")
            
            else:
                rospy.logwarn("Unknown command: %s", cmd)
        except Exception as e:
            rospy.logerr("Command '%s' failed: %s", cmd, repr(e))
...
def main():
    rospy.init_node("rtde_gripper_node")
    GripperNode()
    rospy.spin()
